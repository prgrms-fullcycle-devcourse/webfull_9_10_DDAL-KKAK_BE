...
function validateCreateInput(input: CreateTripInput) {
  if (!input.ownerUserId?.trim()) {
    throw new AppError(400, 'ownerUserId는 필수입니다.');
  }

  if (!input.title?.trim()) {
    throw new AppError(400, '여행 제목은 필수입니다.');
  }

  if (!input.tripCurrencyCode?.trim()) {
    throw new AppError(400, '여행 통화 코드는 필수입니다.');
  }
  ...
  if (input.fixedExchangeRate !== undefined && input.fixedExchangeRate <= 0) {
    throw new AppError(400, '환율은 0보다 커야 합니다.');
  }
  ...
  if (
    ...
    throw new AppError(400, 'startDate 형식이 올바르지 않습니다.');
  }

  if (input.endDate !== undefined && isNaN(new Date(input.endDate).getTime())) {
    throw new AppError(400, 'endDate 형식이 올바르지 않습니다.');
  }
  ...
  if (
    ...
    throw new AppError(400, '종료일은 시작일보다 이후여야 합니다.');
  }
}
